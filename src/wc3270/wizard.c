...
and dash '-')\n");
	...
	sprintf(path, "%s\\%s.wc3270", INSTALL_DIR, cname);
	f = fopen(path, "r");
	if (f != NULL) {
		for (;;) {
	...
	hres = CreateLink(
		INSTALL_DIR "\\wc3270.exe",	/* path to executable */
		linkpath,			/* where to put the link */
		"wc3270 session",		/* description */
		path,				/* arguments */
		INSTALL_DIR,			/* working directory */
