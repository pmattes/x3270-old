...
proxy_socks5(int fd, char *host, unsigned short port, int force_d)
{
	...
	if (force_d)
		...
		rc = getaddrinfo(host, CN, &hints, &res);
		if (rc != 0) {
			use_name = 1;
		} else {
			...
			case AF_INET6:
				...
				break;
			default:
				popup_an_error("SOCKS5 proxy: %s: unknown "
					"family %d",
					host, res->ai_family);
				freeaddrinfo(res);
				return -1;
	    ...
	    hp = gethostbyname(host);
	    if (hp != NULL) {
		    (void) memcpy(&ipaddr, hp->h_addr, hp->h_length);
	    } else {
		    ipaddr.s_addr = inet_addr(host);
		    if (ipaddr.s_addr == (unsigned long)-1)
			    use_name = 1;
	    }
#endif /*]*/
	...
	*s++ = 0x00;		/* reserved */
	if (use_name) {
		...
		*s++ = strlen(host);
		...
		memcpy(s, &ipaddr, 4);
		s += 4;
		strcpy(nbuf, inet_ntoa(ipaddr));
#if defined(AF_INET6) /*[*/
	} else {
	    	*s++ = 0x04;	/* IPv6 */
		memcpy(s, &ip6addr, sizeof(ip6addr));
		s += sizeof(ip6addr);
		(void) inet_ntop(AF_INET6, &ip6addr, nbuf, sizeof(nbuf));
#endif /*]*/
	}
	SET16(s, port);
	...
	trace_dsn("SOCKS5 Proxy: xmit version 5 connect %s %s port %u\n",
		use_name? "domainname": ((af == AF_INET)? "IPv4": "IPv6"),
		use_name? host: nbuf,
		port);
	trace_netdata('>', (unsigned char *)buf, s - buf);
#endif /*]*/
...
#if defined(AF_INET6) /*[*/
	case 0x04: /* IPv6 */
	    	memcpy(&ip6addr, &buf[4], sizeof(struct in6_addr));
		(void) inet_ntop(AF_INET6, &ip6addr, nbuf, sizeof(nbuf));
