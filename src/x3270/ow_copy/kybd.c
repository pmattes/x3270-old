...
Boolean		insert = False;		/* insert mode */
...
{
	if (inhibit) {
		kybdlock_set(KL_ENTER_INHIBIT);
		if (kybdlock == KL_ENTER_INHIBIT)
			status_reset();
	} else {
		kybdlock_clr(KL_ENTER_INHIBIT);
		if (!kybdlock)
			status_reset();
	}
}
...
{
	if (CONNECTED) {
		kybdlock_clr(KL_NOT_CONNECTED);

		/* If in 3270 mode, wait for a WCC(restore) from the host */
		if (!ansi_host || IN_3270)
			kybdlock_set(KL_AWAITING_FIRST);
	} else {
		kybdlock_set(KL_NOT_CONNECTED);
		flush_ta();
	}
}
...
int error_type;
{
	...
	mcursor_locked();
	kybdlock_set((unsigned int)error_type);
	flush_ta();
	if (scripting())
...
Cardinal *num_params;
{
	...
	for (i = 0; i < actioncount; i++)
		if (actions[i].proc == (XtActionProc)action)
			break;
	if (i >= actioncount)
		(void) fprintf(tracef, "(unknown)(");
	else
		(void) fprintf(tracef, "%s(", actions[i].string);
...
unsigned char	aid_code;
{
	...
	mcursor_waiting();
	insert_mode(False);
	kybdlock_set(KL_OIA_TWAIT | KL_OIA_LOCKED);
	aid = aid_code;
...
Cardinal *num_params;
{
	...
	k = atoi(params[0]);
	if (k < 0 || k > PA_SZ) {
		popup_an_error("PA: invalid argument %d", k);
		return;
	}
	if (kybdlock)
...
Cardinal *num_params;
{
	...
	if (toggled(EVENT_TRACE)) {
		(void) fprintf(tracef, " %s -> Key(\"%s\")\n",
		    ia_name[(int) ia_cause], ctl_see((int) cg2asc[cgcode]));
	}
...
XtIntervalId *id;
{
	kybdlock_clr(KL_DEFERRED_UNLOCK);
	status_reset();
	if (CONNECTED)
		ps_process();
}
...
Boolean explicit;
{
	 ...
	 * keyboard now, or want to defer the full time into the future.
	 */
	...
	/*
	 * If explicit (from the keyboard), flush typeahead and unlock
	 * the keyboard now.  Otherwise (from the host), schedule a deferred
	 * keyboard unlock.
	 */
	if (explicit) {
		kybdlock_clr(-1);
		flush_ta();
	} else if (kybdlock & (KL_OIA_TWAIT | KL_OIA_LOCKED | KL_AWAITING_FIRST)) {
		kybdlock_clr(-1);
		kybdlock_set(KL_DEFERRED_UNLOCK);
...
build_composites()
{
	...
	if ((c = get_resource(cn)) == NULL) {
		xs_warning("Compose: Cannot find composeMap \"%s\"",
		    appres.compose_map);
		return False;
	}
	XtFree(cn);
 ...
 * become spaces.  Backslashes are not special.
 *
