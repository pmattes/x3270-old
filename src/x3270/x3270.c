...
int             ov_cols, ov_rows;
char           *programname;
char           *pending_string = CN;
Pixmap          gray;
Boolean         is_olwm = False;
XrmDatabase     rdb;
...
XrmOptionDescRec options[]= {
	...
	{ OptIconName,	".icon",	XrmoptionSepArg,	NULL },
...
char	*argv[];
{
	...
	if (appres.apl_mode && !saw_apl_keymod) {
		find_keymaps(Apl);
		if (appres.key_map)
			appres.key_map = xs_buffer("%s,%s", appres.key_map,
			    Apl);
		else
			appres.key_map = Apl;
	}
...
XEvent *e;
{
	...
	if (e->type == KeymapNotify) {
		ia_cause = IA_PEEK;
		keymap_event((Widget)NULL, e, (String *)NULL, &zero);
	}
}
...
/*
 * Find out if our window manager is olwm.
 */
static Boolean
find_olwm()
{
	...
	a = XInternAtom(display, "_SUN_WM_PROTOCOLS", True);
	if (a == None)
		return False;

	if (XGetWindowProperty(display, root_window, a, 0L, 1L, False,
