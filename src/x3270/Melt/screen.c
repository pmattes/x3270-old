 ...
 * Screen melt.
 */
static Boolean melting = False;
#define MELT_MS		2
#define MELT_PER	50
static int *cand = (int *)NULL;
...
XtIntervalId *id;
{
	...
	for (iter = 0; iter < MELT_PER; iter++) {
		...
		/* Pick a spot and melt it. */
		i = rand() % nc;
		for (j = cand[i]; j >= 0; j -= COLS)
			ctlr_add(j + COLS, screen_buf[j]);
		ctlr_add(j + COLS, CG_null, 0);
		if (appres.m3279)
			ctlr_add_fg(cand[i] + COLS, rcolor());

	}

	(void) XtAppAddTimeOut(appcontext, MELT_MS, melt_down, 0);
}
...
XtIntervalId *id;
{
	...
	for (iter = 0; iter < MELT_PER; iter++) {
		...
		/* Pick a spot and melt it. */
		i = rand() % nc;
		for (j = cand[i]; j < ROWS*COLS; j += COLS)
			ctlr_add(j - COLS, screen_buf[j]);
		ctlr_add(j - COLS, CG_null, 0);
		if (appres.m3279)
			ctlr_add_fg(cand[i] - COLS, rcolor());
	...
	(void) XtAppAddTimeOut(appcontext, MELT_MS, melt_up, 0);
}

/*ARGSUSED*/
void
melt(w, event, params, num_params)
Widget w;
XButtonEvent *event;
String *params;
Cardinal *num_params;
{
	if (melting)
		return;
	cand = (int *)XtMalloc(COLS * sizeof(int));
	srand(time(0L));
	melting = True;
	if (rand() % 2)
		(void) XtAppAddTimeOut(appcontext, MELT_MS, melt_down, 0);
